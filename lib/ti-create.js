...
function create(what) {
    ...
    if (what == "project") {
        ...
        if (fold.getEntriesSync().length > 0) {
            ...
            miniEditor.setAttribute('placeholder-text', "Name");
                ...
                if (e.keyCode == 27) {
                    ...
                    if (str !== "") {
                        ...
                        if (what == "controller") {
                            ...
                            var modelStringField = document.createElement('atom-text-editor');
                            modelStringField.setAttribute('placeholder-text', "Model string: TYPE COLUMN1:TYPE COLUMN2:TYPE ...");
                            modelStringField.setAttribute('mini', true);

                            var inputPanelModel = atom.workspace.addModalPanel({
                                item: modelStringField
                            });
                            modelStringField.focus();

                            modelStringField.onkeyup = function(e) {
                                // get text content
                                var strModel = modelStringField.getModel().getText();
                                if (e.keyCode == 27) {
                                    // ESC
                                    modelStringField.remove();
                                    inputPanelModel.destroy();
                                } else if (strModel.length > 0 && e.keyCode == 13) {
                                    var adapter = strModel.split(" ");
                                    args = ["generate", "model" , str];
                                    args = args.concat(adapter);
                                    runProcess(options, args, what, str);
                                    modelStringField.remove();
                                    inputPanelModel.destroy();
                                }
